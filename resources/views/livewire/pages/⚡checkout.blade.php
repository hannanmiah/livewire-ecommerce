...
<x-layouts::app :title="__('Checkout')">
    <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <nav class="mb-6 flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400">
            ...
            <a href="{{ route('cart') }}" wire:navigate class="hover:text-zinc-700 dark:hover:text-zinc-300">{{ __('Cart') }}</a>
            <flux:icon icon="chevron-right" class="size-3.5" />
        ...
        <div class="mb-8 flex items-center gap-2">
            @foreach([1 => __('Shipping'), 2 => __('Billing'), 3 => __('Review')] as $num => $label)
                @if($num > 1)
                ...
                @if($step === 1)
                    <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
                        ...
                        @if($this->shippingAddresses->isNotEmpty())
                                ...
                                @foreach($this->shippingAddresses as $address)
                                    <label
                                        ...
                                        @class([
                                            ...
                                            'border-zinc-900 bg-zinc-50 dark:border-zinc-100 dark:bg-zinc-700' => $selected_shipping_address === $address->id,
                                            'border-zinc-200 bg-white hover:border-zinc-300 dark:border-zinc-700 dark:bg-zinc-800 dark:hover:border-zinc-600' => $selected_shipping_address !== $address->id,
                                        ])
                                    >
                                        <flux:radio
                                            wire:model="selected_shipping_address"
                                            value="{{ $address->id }}"
                                            name="selected_shipping_address"
                                        />
                                        <div class="flex-1">
                                            <flux:text variant="strong">{{ $address->address_line }}</flux:text>
                                            <flux:text class="text-sm">{{ $address->city }}, {{ $address->country }} {{ $address->postal_code }}</flux:text>
                                            @if($address->is_default)
                                                <flux:badge color="blue" size="sm" class="mt-1">{{ __('Default') }}</flux:badge>
                                            @endif
                                        </div>
                                    </label>
                                @endforeach
                            ...
                            <div class="mb-4">
                                <flux:text class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Or add a new address:') }}</flux:text>
                            </div>
                        @endif
                        ...
                        <div class="space-y-4">
                            <flux:field>
                                ...
                                <flux:input wire:model="shipping_address_line" placeholder="{{ __('123 Main Street') }}" />
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <flux:field>
                                    ...
                                    <flux:input wire:model="shipping_postal_code" placeholder="{{ __('Postal Code') }}" />
                    ...
                    <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
                        ...
                        @if(! $billing_same_as_shipping)
                            ...
                            <div class="space-y-4">
                                <flux:field>
                                    <flux:label>{{ __('Address Line') }}</flux:label>
                                    <flux:input wire:model="billing_address_line" placeholder="{{ __('123 Main Street') }}" />
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <flux:field>
                                        <flux:label>{{ __('Postal Code') }}</flux:label>
                                        <flux:input wire:model="billing_postal_code" placeholder="{{ __('Postal Code') }}" />
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
                            ...
                            @if($selected_shipping_address)
                                ...
                                <flux:text class="text-sm">{{ $sa->city }}, {{ $sa->country }} {{ $sa->postal_code }}</flux:text>
                            @else
                                <flux:text>{{ $shipping_address_line }}</flux:text>
                                <flux:text class="text-sm">{{ $shipping_city }}, {{ $shipping_country }} {{ $shipping_postal_code }}</flux:text>
                            @endif
                        </div>
                        ...
                        <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
                            ...
                            @if($billing_same_as_shipping)
                                <flux:text class="text-sm text-zinc-500">{{ __('Same as shipping address') }}</flux:text>
                            @elseif($selected_billing_address)
                                ...
                                <flux:text class="text-sm">{{ $ba->city }}, {{ $ba->country }} {{ $ba->postal_code }}</flux:text>
                            @else
                                <flux:text>{{ $billing_address_line }}</flux:text>
                                <flux:text class="text-sm">{{ $billing_city }}, {{ $billing_country }} {{ $billing_postal_code }}</flux:text>
                            @endif
                        </div>
                        ...
                        <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
                            ...
                            <div class="divide-y divide-zinc-200 dark:divide-zinc-700">
                                @foreach($cart->items as $item)
                                    <div wire:key="review-item-{{ $item->id }}" class="flex items-center gap-4 py-4 first:pt-0 last:pb-0">
                                        {{-- Product Image --}}
                                        ...
                                        <div class="text-right">
                                            ...
                                            @if($item->quantity > 1)
                                                <flux:text class="block text-xs text-zinc-500 dark:text-zinc-400">${{ number_format((float) $item->price, 2) }} {{ __('each') }}</flux:text>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    <div class="sticky top-8 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
                        ...
                        @if($cart->coupon_code)
                            <div class="mt-4 flex items-center gap-2 rounded-md bg-green-50 px-3 py-2 dark:bg-green-900/20">
                                ...
                                <flux:text class="text-xs text-green-700 dark:text-green-300">{{ __('Coupon: :code', ['code' => $cart->coupon_code]) }}</flux:text>
                            </div>
                        @endif
