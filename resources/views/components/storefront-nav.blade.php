...
<div>
    <flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navbar class="me-1.5 space-x-0.5 rtl:space-x-reverse py-0!">
            <form action="{{ route('products.index') }}" method="GET" wire:navigate class="flex items-center"
                  x-data="{ search: new URLSearchParams(window.location.search).get('search') || '' }">
                <flux:input
                        x-model="search"
                        name="search"
            ...
            @else
                <flux:tooltip :content="__('Log in')" position="bottom">
                    <flux:navbar.item class="!h-10 [&>div>svg]:size-5" icon="arrow-right-start-on-rectangle"
                                      :href="route('login')" :label="__('Log in')"/>
                </flux:tooltip>
                <flux:button
                        variant="primary"
                        size="sm"
    ...
    <flux:sidebar collapsible="mobile" sticky
        ...
        <flux:sidebar.nav>
            <flux:sidebar.group :heading="__('Shop')" class="grid">
                ...
                @foreach($this->featuredCategories as $category)
                    <flux:sidebar.item icon="folder" :href="route('products.by-category', $category->slug)"
                                       wire:navigate>
                        {{ $category->name }}
                    </flux:sidebar.item>
                @endforeach
        ...
        <flux:sidebar.nav>
            <flux:sidebar.item icon="shopping-cart" :href="route('cart')" wire:navigate>
                {{ __('Cart') }}
                @if($this->cartCount > 0)
                    <flux:badge color="primary" size="sm" class="ml-2">{{ $this->cartCount }}</flux:badge>
                @endif
            </flux:sidebar.item>
